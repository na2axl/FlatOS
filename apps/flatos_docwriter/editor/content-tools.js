...
(function () {
  ...
  ContentEdit.PreText = (function (_super) {
    ...
    PreText.prototype._keyTab = function (ev) {
      ...
      cursor = selection.get()[0] + 1;
      if (selection.get()[0] === 0 && selection.isCollapsed()) {
        this.content = new HTMLString.String('\t', true).concat(this.content);
      } else if (this._atEnd(selection) && selection.isCollapsed()) {
        this.content = this.content.concat(new HTMLString.String('\t', true));
      } else if (selection.get()[0] === 0 && selection.get()[1] === this.content.length()) {
        this.content = new HTMLString.String('\t', true);
        cursor = 0;
      } else {
        tip = this.content.substring(0, selection.get()[0]);
        tail = this.content.substring(selection.get()[1]);
        this.content = tip.concat(new HTMLString.String('\t', true), tail);
      }
      this.updateInnerHTML();
      selection.set(cursor, cursor);
      selection.select(this._domElement);
...
(function () {
  ...
  ContentTools.ImageDialog = (function (_super) {
    ...
    ImageDialog.prototype.mount = function () {
      ...
      this._domFlipV = this.constructor.createDiv(['ct-control', 'ct-control--icon', 'ct-control--clip-vertical']);
      this._domFlipV.setAttribute('data-ct-tooltip', ContentEdit._('Flip Vertically'));
      domTools.appendChild(this._domFlipV);
      this._domFlipH = this.constructor.createDiv(['ct-control', 'ct-control--icon', 'ct-control--clip-horizontal']);
      this._domFlipH.setAttribute('data-ct-tooltip', ContentEdit._('Flip Horizontally'));
      domTools.appendChild(this._domFlipH);
  ...
  _EditorApp = (function (_super) {
    ...
    _EditorApp.prototype.app = function () {
      return this._domElement;
    };
  ...
  ContentTools.Tools.FontSize = (function (_super) {
    __extends(FontSize, _super);

    function FontSize() {
      return FontSize.__super__.constructor.apply(this, arguments);
    }

    // ContentTools.ToolShelf.stow(FontSize, 'fontsize');

    FontSize.label = 'FontSize';

    FontSize.icon = 'fontsize';

    FontSize.tagName = 'font';

    FontSize.canApply = function (element, selection) {
      if (!element.content) {
        return false;
      }
    ...
    FontSize.isApplied = function (element, selection) {
      var from, to, _ref;
      ...
      this.$element = $(HTMLString.String.decode(element.content.slice(from, to).html()));
      return element.content.slice(from, to).hasTags(this.tagName, true) && this.$element.hasClass('fontsize');
    };

    FontSize.apply = function (element, selection, callback) {
      var from, to, _ref;
      element.storeState();
      _ref = selection.get(), from = _ref[0], to = _ref[1];
      this.$element = $(HTMLString.String.decode(element.content.slice(from, to).html()));
      var size = parseInt(this.$element.css('font-size')) + this.factor;
      size = (size > 8) ? size : 8;
      var tag = new HTMLString.Tag(this.tagName, { style: 'font-size:' + size + 'px', 'class': 'fontsize' });
      if (this.isApplied(element, selection)) {
        element.content = element.content.unformat(from, to, new HTMLString.Tag(this.tagName, { style: 'font-size:' + (size-this.factor) + 'px', 'class': 'fontsize' }));
      }
  ...
  ContentTools.Tools.FontSizeUp = (function (_super) {
    ...
    ContentTools.ToolShelf.stow(FontSizeUp, 'fontsizeup');

    FontSizeUp.label = 'FontSize Up';

    FontSizeUp.icon = 'fontsizeup';

    FontSizeUp.factor = 2;

    return FontSizeUp;

  })(ContentTools.Tools.FontSize);
  ...
  ContentTools.Tools.FontSizeDown = (function (_super) {
    ...
    ContentTools.ToolShelf.stow(FontSizeDown, 'fontsizedown');

    FontSizeDown.label = 'FontSize Down';

    FontSizeDown.icon = 'fontsizedown';

    FontSizeDown.factor = -2;
  ...
  ContentTools.Tools.Unformat = (function (_super) {
    __extends(Unformat, _super);

    function Unformat() {
      return Unformat.__super__.constructor.apply(this, arguments);
    }

    ContentTools.ToolShelf.stow(Unformat, 'unformat');

    Unformat.label = 'Unformat';

    Unformat.icon = 'unformat';

    Unformat.tagNames = ['b', 'i', 'u', 'strike', 'sub', 'sup', 'font'];

    Unformat.canApply = function (element, selection) {
      if (!element.content) {
        return false;
      }
      return selection && !selection.isCollapsed();
    ...
    Unformat.isApplied = function (element, selection) {
      var from, to, _ref, isApplied;
      if (element.content === void 0 || !element.content.length()) {
        return false;
      }
      _ref = selection.get(), from = _ref[0], to = _ref[1];
      ...
      isApplied = true;
      for (var i = 0, l = this.tagNames.length; i < l; i++) {
        if (element.content.slice(from, to).hasTags(this.tagNames[i], true)) {
          isApplied = false;
        }
      }
      return isApplied;
    ...
    Unformat.apply = function (element, selection, callback) {
      var from, to, _ref;
      element.storeState();
      _ref = selection.get(), from = _ref[0], to = _ref[1];
      if (!this.isApplied(element, selection)) {
        for (var i = 0, l = this.tagNames.length; i < l; i++) {
          element.content = element.content.unformat(from, to, new HTMLString.Tag(this.tagNames[i]));
        }
      }
      element.content.optimize();
      element.updateInnerHTML();
  ...
  ContentTools.Tools.AlignLeft = (function (_super) {
    ...
    AlignLeft.apply = function (element, selection, callback) {
      ...
      alignmentClassNames = [ContentTools.Tools.AlignLeft.className, ContentTools.Tools.AlignCenter.className, ContentTools.Tools.AlignRight.className, ContentTools.Tools.AlignFull.className];
      for (_i = 0, _len = alignmentClassNames.length; _i < _len; _i++) {
        className = alignmentClassNames[_i];
        if (element.hasCSSClass(className)) {
          element.removeCSSClass(className);
  ...
  ContentTools.Tools.AlignFull = (function (_super) {
    __extends(AlignFull, _super);

    function AlignFull() {
      return AlignFull.__super__.constructor.apply(this, arguments);
    }

    ContentTools.ToolShelf.stow(AlignFull, 'align-justify');

    AlignFull.label = 'Justify';

    AlignFull.icon = 'align-justify';

    AlignFull.className = 'text-justify';
  ...
  ContentTools.Tools.UnorderedList = (function (_super) {
    ...
    UnorderedList.isApplied = function (element, selection) {
      var list = element.closest(function (node) {
        return node.type() === 'List';
      });
      return element.parent().type() === 'ListItem' && list.tagName() === this.listTag;
    };

    UnorderedList.apply = function (element, selection, callback) {
      var insertAt, list, listItem, listItemText, parent, p;
